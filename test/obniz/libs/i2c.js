...
describe("obniz.libs.i2c", function () {
  ...
  it("start",  function () {
    this.obniz.i2c0.start({mode:"master" , sda:2,scl: 3, clock:400000, pull:null}); 
  ...
  it("end",  function () {
    this.obniz.i2c0.start({mode:"master" , sda:2,scl: 3, clock:400000, pull:"5v"}); 
    expect(this.obniz).send({ io2: { output_type: 'open-drain' } });
    expect(this.obniz).send({ io3: { output_type: 'open-drain' } });
  ...
  it.skip("write10bit フォーマット変更すべき",  function () {
    this.obniz.i2c0.start({mode:"master" , sda:2,scl: 3, clock:100000, pull:"3v"}); 
    expect(this.obniz).send({ io2: { output_type: 'open-drain' } });
    expect(this.obniz).send({ io3: { output_type: 'open-drain' } });
    expect(this.obniz).send({io2:{"pull_type":"pull-up5v"}});
    expect(this.obniz).send({io3:{"pull_type":"pull-up5v"}});
    expect(this.obniz).send({i2c0:{"clock": 100000, "sda": 2, "scl":3,"mode":"master" }});
  ...
  it.skip("readWait withothers",  function () {
    this.obniz.i2c0.start({mode:"master" , sda:2,scl: 3, clock:100000, pullType:"pull-up3v"}); 
    expect(this.obniz).send({ io2: { output_type: 'open-drain' } });
    expect(this.obniz).send({ io3: { output_type: 'open-drain' } });
    expect(this.obniz).send({io2:{"pull_type":"pull-up3v"}});
    expect(this.obniz).send({io3:{"pull_type":"pull-up3v"}});
