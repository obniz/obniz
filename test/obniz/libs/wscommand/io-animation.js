...
describe("io.animation", function () {
  ...
  it("request ioAnimation", function () {
    compressTest(
      this.obniz,
      [{ "io": { "animation": { "name": "animation-1", "states": [{ "duration": 10, "state": { "io0": false, "io1": true } }, { "duration": 10, "state": { "io0": true, "io1": false } }], "status": "loop" } } }],
...
function compressTest(obniz, requestJson, expecteBinarystrings) {
  let binaryArray = expecteBinarystrings[0].split(" ").map(function (val, index) { return parseInt(val, 16); });
  ...
  let compress = obniz.constructor.WSCommand.compress(obniz.wscommands, requestJson[0]);
