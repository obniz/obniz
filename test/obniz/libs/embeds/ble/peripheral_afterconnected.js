...
describe("ble", function () {
  ...
  it("onconnection updates", function () {
    ...
    expect(this.obniz.ble.peripheral.onconnectionupdates.getCall(0).args.length).to.be.equal(1);
    expect(this.obniz.ble.peripheral.onconnectionupdates.getCall(0).args[0]).to.be.deep.equal({
      "address": "77e754ab8591",
      "status": "connected"
    });
    expect(this.obniz).to.be.finished;
  ...
  it("unknown  descriptor", function () {
    ...
    let desciptor = service.getCharacteristic("0000");
