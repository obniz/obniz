...
class SNx4HC595Test {
  public ioNum() {
    ...
    obniz.onconnect = async () => {
      const ioext = obniz.wired("SNx4HC595", { gnd: 0, vcc: 1, ser: 2, rclk: 3, srclk: 4 });
      ioext.ioNum(16);
  ...
  public getIO() {
    ...
    obniz.onconnect = async () => {
      ...
      const seg = obniz.wired("7SegmentLED", { a: io3, b: io4, c: 5, d: 6, e: 7, f: 8, g: 9, common: 10 });
      seg.print(0);
    };
  }
  ...
  public setEnable() {
    const obniz = new Obniz(OBNIZ_ID);
    obniz.onconnect = async () => {
      const ioext = obniz.wired("SNx4HC595", { gnd: 0, vcc: 1, ser: 2, rclk: 3, srclk: 4, oe: 5, enabled: false });
