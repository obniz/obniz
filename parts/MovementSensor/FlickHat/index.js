class FlickHat {
  ...
  async start(callbackFwInfo) {
    ...
    this.i2c.write(this.address, [0x10, 0x00, 0x00, 0xa2, 0xa1, 0x00, 0x00, 0x00, 0x1f, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff]);
    await this.obniz.wait(100);

    this.i2c.write(this.address, [0x10, 0x00, 0x00, 0xa2, 0x80, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x00, 0x00]);
  }
            ...
            if (configmask & maskGestureInfo && gesture[0] > 0) {
              ...
              for (let index in gestures) {
                if (index == gesture[0] && typeof this.ongestureall == 'function')
                  this.ongestureall({action: gestures[index][0], from: gestures[index][1], to: gestures[index][2], raw: gesture, seq: seq});
                if (index == gesture[0] && gestures[index][0] == 'flick' && typeof this.ongesture == 'function')
                  this.ongesture({action: 'gesture', from: gestures[index][1], to: gestures[index][2], raw: gesture, seq: seq});
              }
            }
...
//console.log(`touchAction:${touchAction.toString(16)}, comp:${comp.toString(16)}, index:${index}, group:${group}`);
              ...
              if (touches.length > 0 && typeof this.ontouch == 'function') 
                this.ontouch({action: 'touch', positions: touches, raw: touch, seq: seq});

              if (taps.length > 0 && typeof this.ontap == 'function') 
                this.ontap({action: 'tap', positions: taps, raw: touch, seq: seq});

              if (doubletaps.length > 0 && typeof this.ondoubletap == 'function') 
                this.ondoubletap({action: 'doubletap', positions: doubletaps, raw: touch, seq: seq});
            ...
            if (configmask & maskAirWheelInfo && sysinfo & sysAirWheelValid) {
              ...
              if (delta != 0 && delta > -0.5 && delta < 0.5) {
                if (typeof this.onairwheel == 'function') this.onairwheel({delta: delta * 360.0, rotation: this.rotation, raw: airwheel, seq: seq});
              }
