...
class MFRC522 {
	...
	async init() {	// Initializes the MFRC522 chip
		// Hardware and Software reset
		this.RST.output(false);
		await this.obniz.wait(50);	// 8.8.2 says the oscillator start-up time is the start up time of the crystal + 37,74μs: 50ms.
		...
		this.writeRegister(this.TPrescalerReg, 0xA9);	// TPreScaler = TModeReg[3..0]: TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25μs.
	...
	writeRegister(addr, val) {
		let data;
		if (val instanceof Array) {	// If val is Array
	...
	async readRegister_nByte(addr, n) {
		let dataArray = [];
		if (addr instanceof Array) {	// Multiple addresses(If addr is Array)
			for (let i = 0; i < addr.length; i++) {
				dataArray.push(((addr[i] << 1) & 0x7E) | 0x80);
			};
		} else {	// Single address & read n times
			for (let i = 0; i < n; i++) {
	...
	async antennaOn() {	// Turns the antenna on by enabling pins TX1 and TX2
		let response = await this.readRegister(this.TxControlReg);
		if ((response & 0x03) !== 0x03) {	//If TX1 and TX2 down
			await this.setRegisterBitMask(this.TxControlReg, response | 0x03);
		}
	}

	async antennaOff() { // Turns the antenna off by disabling pins TX1 and TX2
	...
	async toCard(command, bitsToSend) {
		...
		do {	//Wait for the received data complete
		...
		// await this.clearRegisterBitMask(this.BitFramingReg, 0x80);	//resetAndInit()でついでにReset

		const response = await this.readRegister_nByte([this.ErrorReg, this.FIFOLevelReg, this.ControlReg]);

		if (TryingTimes !== 0) {
			if ((response[0] & 0x1B) === 0x00) {	// BufferOvfl CollErr ParityErr ProtocolErr
	...
	async findCard() {
		...
		const PICC_Type = await this.identifyCardType(uid);
	...
	async identifySoftware() {
		let version = await this.readRegister(this.VersionReg);
		switch (version) {
			case 0x88: version = "(clone)"; break;
			case 0x90: version = "v0.0"; break;
			case 0x91: version = "v1.0"; break;
			case 0x92: version = "v2.0"; break;
			case 0x12: version = "counterfeit chip"; break;
	...
	async identifyCardType(uid) {	// Identify type of the scanned card
		let buffer = [this.PICC_SElECTTAG, 0x70].concat(uid);
		buffer = buffer.concat(await this.calculateCRC(buffer));
		...
		switch (PICC_Type) {
			case 0x04: PICC_Type = "SAK indicates UID is not complete."; break;	// UID not complete
			case 0x09: PICC_Type = "MIFARE Mini, 320 bytes"; break;
			case 0x08: PICC_Type = "MIFARE 1KB"; break;
			case 0x18: PICC_Type = "MIFARE 4KB"; break;
			case 0x00: PICC_Type = "MIFARE Ultralight or Ultralight C"; break;
			case 0x11: PICC_Type = "MIFARE Plus"; break;
			case 0x01: PICC_Type = "MIFARE TNP3XXX"; break;
			case 0x20: PICC_Type = "PICC compliant with ISO/IEC 14443-4"; break;
			case 0x40: PICC_Type = "PICC compliant with ISO/IEC 18092 (NFC)"; break;
			default: throw "PICC_type_ERROR";
		}
		return PICC_Type;
	}

	async readSectorData(Sector, uid) {
		await this.authenticateSector(Sector, uid);
		return await this.getSectorData(Sector);
	}

	async readBlockData(Block, uid) {
		await this.authenticateBlock(Block, uid);
		return await this.getBlockData(Block);
	}

	async authenticateSector(Sector, uid) {
	...
	async authenticateBlock(Block, uid) {
		/* Password authentication mode (A or B)
	...
	async readAgain() {	// If you finish reading and want to read again, this can use instead of init()
		await this.clearRegisterBitMask(this.Status2Reg, 0x08);
	}

	async getSectorData(address) {
		let response = [];
		let blockData = [];
		for (let i = 0; i < 4; i++) {
			let request = [this.PICC_READ, address * 4 + i];
			request = request.concat(await this.calculateCRC(request));
	...
	async writeBlockData(Block, sixteenBytes) {
		if (Block === 0 || Block % 4 === 3) {
			throw "deny_Write"
		}
		const buffer = [this.PICC_WRITE, Block];
		let response = await this.appendCRCtoBufferAndSendToCard(buffer);
		if (response.status) {
			response = await this.appendCRCtoBufferAndSendToCard(sixteenBytes);
		}
		else
