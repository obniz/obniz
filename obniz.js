...
class BME280 {
  ...
  wired(obniz) {
    ...
    if (obniz.isValidIO(this.params.csb)) { // selecting I2C mode before powerup
  ...
  async config() {
    this.write([this.commands.addresses.config,     (this.configration.interval << 5) | (this.configration.iir_strength << 2) | 0]);
    this.write([this.commands.addresses.ctrl_hum,    this.configration.sampling.hum]);
    this.write([this.commands.addresses.ctrl_meas,  (this.configration.sampling.temp << 5) | (this.configration.sampling.pres << 2) | this.configration.mode]);
  }
  ...
  calibration_H(adc_H){
    ...
    h = (adc_H - (this._calibrated.dig_H4 * 64 + this._calibrated.dig_H5 / 16384 * h)) *
        (this._calibrated.dig_H2 / 65536 * (1 + this._calibrated.dig_H6 / 67108864 * h * (1 + this._calibrated.dig_H3 / 67108864 * h)));
