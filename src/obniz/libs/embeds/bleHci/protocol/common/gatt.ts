...
export class GattCommon {
  ...
  readRequest(handle: HandleIndex) {
    const buf = Buffer.alloc(3);
  ...
  readBlobRequest(handle: HandleIndex, offset: number) {
    const buf = Buffer.alloc(5);
  ...
  findInfoRequest(startHandle: HandleIndex, endHandle: HandleIndex) {
    const buf = Buffer.alloc(5);
  ...
  writeRequest(handle: HandleIndex, data: Buffer, withoutResponse: boolean) {
  ...
  prepareWriteRequest(handle: HandleIndex, offset: number, data: Buffer) {
  ...
  executeWriteRequest(handle: HandleIndex, cancelPreparedWrites?: boolean) {
