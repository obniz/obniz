...
export class W5500 implements ObnizPartsInterface {
  ...
  protected interruptHandlers: { [key in W5500.Interrupt]?: ((ethernet: W5500) => Promise<void>) | ((ethernet: W5500, extra?: W5500.DestInfo) => Promise<void>) } = {};
   ...
   * @return 書き込み結果
   */
  public readonly init = async (option: W5500.Options) => {
    // リセット
    ...
    // ネットワークの初期化
    if (option.gatewayIP) result = result && (await this.setGateway(option.gatewayIP));
    if (option.subnetMask) result = result && (await this.setSubnetMask(option.subnetMask));
    if (option.macAddress) result = result && (await this.setMacAddress(option.macAddress));
    if (option.localIP) result = result && (await this.setLocalIP(option.localIP));

    // 割り込みマスクを全てオンに
    result = result && (await this.setInterruptMask(0xFF));

    // その他設定
    if (option.retryTime) result = result && (await this.setRetryTime(option.retryTime));
    if (option.retryCount) result = result && (await this.setRetryCount(option.retryCount));
    if (option.linkControlProtocolRequestTimer) result = result && (await this.setPPPLinkControlProtocolRequestTimer(option.linkControlProtocolRequestTimer));
    if (option.linkControlProtocolMagicNumber) result = result && (await this.setPPPLinkControlProtocolMagicNumber(option.linkControlProtocolMagicNumber));
    if (option.pppoeDestMACAddress) result = result && (await this.setPPPoEMacAddress(option.pppoeDestMACAddress));
    if (option.pppoeSessionID) result = result && (await this.setPPPoESessionID(option.pppoeSessionID));
    if (option.pppoeMaxSegmentSize) result = result && (await this.setPPPoEMaxSegmentSize(option.pppoeMaxSegmentSize));
    if (option.phyConfig) result = result && (await this.setPhysicalConfig(option.phyConfig));

    // 割り込みハンドラー設定
    if (option.onIPConflictInterrupt) this.setInterruptHandler('IPConflict', option.onIPConflictInterrupt);
    if (option.onDestUnreachInterrupt) this.setInterruptHandler('DestUnreach', option.onDestUnreachInterrupt);
    if (option.onPPPoECloseInterrupt) this.setInterruptHandler('PPPoEClose', option.onPPPoECloseInterrupt);
    if (option.onMagicPacketInterrupt) this.setInterruptHandler('MagicPacket', option.onMagicPacketInterrupt);
    if (option.onAllInterrupt) this.setAllInterruptHandler(option.onAllInterrupt);
  ...
  public readonly finalize = async () => {
    ...
    for (let f in funcList) await funcList[f]();
    this.spi.end();
   ...
   */
  public readonly setInterruptHandler = (name: W5500.Interrupt, handler: ((ethernet: W5500) => Promise<void>) | ((ethernet: W5500, extra?: W5500.DestInfo) => Promise<void>)) => this.interruptHandlers[name] = handler;
   ...
   */
  public readonly setAllInterruptHandler = (handler: (ethernet: W5500, name: W5500.Interrupt, extra?: W5500.DestInfo) => Promise<void>) => this.allInterruptHandler = handler;

  /**
   * ルーターとの接続が確立されるまで待機
   * @return 物理層のステータス
   */
  public readonly waitLinkUp = async () => {
    let phy;
    while (true) {
   ...
   */
  public readonly getSocket = (socketId: number) => {
    if (socketId < 0 || socketId > 7) throw new Error('Socket id must take a value between 0 and 7.');
   ...
   * @return W5500.Socketのインスタンス
   */
  public readonly getNewSocket = () => {
    let id = 0;
    while (this.socketList[id] !== undefined) id++;
    if (id > 7) return null;
    else return this.socketList[id] = new W5500.Socket(id, this);
  };
   ...
   * @return SPIのステータス
   */
  public readonly getSpiStatus = () => this.spiStatus;
  
  /**
   * W5500をハードウェア的にリセット
   */
  public readonly hardReset = async () => {
   ...
   * @return 書き込み結果
   */
  public readonly setCommonMode = (option: W5500.Options) =>
    this.numWrite(COMMON_MODE, BSB_COMMON,
   ...
   * @return 書き込み結果
   */
  public readonly setGateway = (ip: string) => this.ipWrite(COMMON_GATEWAY_ADDRESS, BSB_COMMON, ip);
   ...
   * @return 書き込み結果
   */
  public readonly setSubnetMask = (mask: string) => this.ipWrite(COMMON_SOURCE_SUBNET_MASK, BSB_COMMON, mask);
   ...
   * @return 書き込み結果
   */
  public readonly setMacAddress = (mac: string) => this.macWrite(COMMON_SOURCE_HARDWARE_ADDRESS, BSB_COMMON, mac);
   ...
   * @return 書き込み結果
   */
  public readonly setLocalIP = (ip: string) => this.ipWrite(COMMON_SOURCE_IP_ADDRESS, BSB_COMMON, ip);
   ...
   * @return 常にtrue
   */
  public readonly checkInterrupt = async (disableAllSocketCheck?: boolean) => {
    if (!this.spiStatus) return false;

    const interrupt = await this.numRead(COMMON_INTERRUPT, BSB_COMMON);
    if (interrupt !== 0) await this.numWrite(COMMON_INTERRUPT, BSB_COMMON, interrupt); //リセット

    const msgList = Object.keys(W5500.InterruptFlags).filter(msg => (interrupt & W5500.InterruptFlags[msg as W5500.Interrupt]) !== 0);
    const extra = msgList.indexOf('DestUnreach') >= 0 ? new W5500.DestInfo(await this.getUnreachableIP(), await this.getUnreachablePort()) : undefined;

    if (disableAllSocketCheck !== false) {
      const funcList = this.socketList.filter(s => s !== undefined && s.getProtocol() !== null).map(s => s.checkInterrupt);
      for (let f in funcList) await funcList[f]();
    ...
    for (let m in msgList) {
      ...
      if (handler !== undefined) await handler(this, extra);
      if (this.allInterruptHandler !== undefined) await this.allInterruptHandler(this, msg, msg === 'DestUnreach' ? extra : undefined);
    }
   ...
   * @return 書き込み結果
   */
  public readonly setInterruptMask = (mask: number) => this.numWrite(COMMON_INTERRUPT_MASK, BSB_COMMON, mask);
   ...
   * @return 書き込み結果
   */
  public readonly setRetryTime = (time: number) => this.num2Write(COMMON_RETRY_TIME, BSB_COMMON, time*10);
   ...
   * @return 書き込み結果
   */
  public readonly setRetryCount = (count: number) => this.numWrite(COMMON_RETRY_COUNT, BSB_COMMON, count);
   ...
   * @return 書き込み結果
   */
  public readonly setPPPLinkControlProtocolRequestTimer = (timer: number) => this.numWrite(COMMON_PPP_LCP_REQUEST_TIMER, BSB_COMMON, timer/25);
   ...
   * @return 書き込み結果
   */
  public readonly setPPPLinkControlProtocolMagicNumber = (num: number) => this.numWrite(COMMON_PPP_LCP_MAGIC_NUMBER, BSB_COMMON, num);
   ...
   * @return 書き込み結果
   */
  public readonly setPPPoEMacAddress = (mac: string) => this.macWrite(COMMON_PPP_DESTINATION_MAC_ADDRESS, BSB_COMMON, mac);
   ...
   * @return 書き込み結果
   */
  public readonly setPPPoESessionID = (id: number) => this.num2Write(COMMON_PPP_SESSION_IDENTIFICATION, BSB_COMMON, id);
   ...
   * @return 書き込み結果
   */
  public readonly setPPPoEMaxSegmentSize = (size: number) => this.num2Write(COMMON_PPP_MAXIMUM_SEGMENT_SIZE, BSB_COMMON, size);

  /**
   * 到達できなかった時のIPアドレスを取得
   * @return IPv4アドレス
   */
  public readonly getUnreachableIP = () => this.ipRead(COMMON_UNREACHABLE_IP_ADDRESS, BSB_COMMON);

  /**
   * 到達できなかった時のポート番号を取得
   * @return ポート番号
   */
  public readonly getUnreachablePort = () => this.num2Read(COMMON_UNREACHABLE_PORT, BSB_COMMON);

  /**
   * 物理層のステータス取得
   * @return 物理層のステータス
   */
  public readonly getPhysicalConfig = async (): Promise<W5500.PhysicalLayerStatus> => {
   ...
   * @return 書き込み結果
   */
  public readonly setPhysicalConfig = async (config: W5500.PhysicalLayerOptions) => {
    if (config.reset) {
      await this.numWrite(COMMON_PHY_CONFIGURATION, BSB_COMMON, 0);
      await sleep(500);
    }
    let value = 0b11111000;
    if (config.autoNegotiation === false) value &= 0b11011000; //1bit目を0に
    if (config.speed === 10)              value &= 0b11001000; //1,2bit目を0に
    if (config.duplex === false)          value &= 0b11110000; //3bit目を0に
    if (config.powerDown === true)         value = 0b11110000;
   ...
   * @return チップバージョン
   */
  public readonly getVersion = () => this.numRead(COMMON_CHIP_VERSION, BSB_COMMON);
   ...
   * @hidden
   */
  public readonly ipWrite = (address: number, bsb: number, ip: string) => this.addressWrite(address, bsb, ip, 'IP Address', '123.234.0.1', '.', 4, 10);
   ...
   * @hidden
   */
  public readonly macWrite = (address: number, bsb: number, mac: string) => this.addressWrite(address, bsb, mac, 'MAC Address', '12:34:56:78:90:AB', ':', 6, 16);
   ...
   * @hidden
   */
  private readonly addressWrite = (address: number, bsb: number, val: string, name: string, example: string, splitVal: string, length: number, radix: number) => {
    if (typeof val !== 'string') throw new Error(`Given ${name} must be string.`);
    const valList = val.split(splitVal).map(addr => parseInt(addr, radix));
    if (valList.filter(addr => typeof addr === 'number').length !== length) throw new Error(`${name} format must be '${example}'.`);
   ...
   * @hidden
   */
  public readonly bigWrite = async (address: number, bsb: number, data: number[], noWait?: boolean) => {
    const maxLength = this.fdm ? 4 : 1021; // FDM(4) / VDM(1024-3)
    let result = true;
    for (let i = 0; i < data.length; i += maxLength) {
      let size = i + maxLength <= data.length ? maxLength : data.length - i;
   ...
   * @hidden
   */
  public readonly numWrite = (address: number, bsb: number, num: number) => this.write(address, bsb, [num]);
   ...
   * @hidden
   */
  public readonly num2Write = (address: number, bsb: number, num: number) => this.write(address, bsb, [(num & 0xFF00) >> 8, num & 0xFF]);
   ...
   * @hidden
   */
  private readonly write = async (address: number, bsb: number, data: number[], noWait?: boolean) => {
    if (!Array.isArray(data)) throw new Error('Given data must be array.');
    if (data.length === 3 && this.fdm) data[3] = 0;
    if (data.length === 0) throw new Error('Given data is empty.');
    if (data.length > 4 && this.fdm) throw new Error('Given data length must be 1, 2 or 4.');
    if (data.length > 1021 && !this.fdm) throw new Error('Given data length must be 1021 or less.');
    if (data.filter(addr => 0x00 <= addr && addr <= 0xFF).length !== data.length) throw new Error('Given data field must take a value between 0(0x00) and 255(0xFF).');
    const result = await this.send(address, bsb, 'Write', data, noWait);
    if (typeof result === 'object') throw new Error('Unexpected Result');
    else return result;
  };
   ...
   * @hidden
   */
  public readonly ipRead = async (address: number, bsb: number) => (await this.read(address, bsb, 4)).join('.');
   ...
   * @hidden
   */
  public readonly bigRead = async (address: number, bsb: number, length: number): Promise<number[]> => {
    const maxLength = this.fdm ? 4 : 1021; // FDM(4) / VDM(1024-3)
    let data: number[] = [];
    for (let i = 0; i < length; i += maxLength) {
      let size = i + maxLength <= length ? maxLength : length - i;
   ...
   * @hidden
   */
  public readonly numRead = async (address: number, bsb: number) => {
   ...
   * @hidden
   */
  public readonly num2Read = async (address: number, bsb: number) => {
   ...
   * @hidden
   */
  private readonly read = async (address: number, bsb: number, length: number) => {
   ...
   * @hidden
   */
  private readonly send = async (address: number, bsb: number, mode: SendMode, data: number[], noWait?: boolean) => {
    let write = [
...
export namespace W5500 {
  ...
  export class Socket {
    ...
    /** 割り込みをメッセージ別でキャッチするハンドラーを保持 */
    protected interruptHandlers: { [key in Socket.Interrupt]?: ((socket: Socket) => Promise<void>) | ((socket: Socket, extra?: number[] | string | W5500.DestInfo) => Promise<void>) } = {};
    /** 割り込みを全てキャッチするハンドラーを保持 */
    protected allInterruptHandler?: (socket: Socket, msg: Socket.Interrupt, extra?: number[] | string | W5500.DestInfo) => Promise<void>;
    /** バイト列 -> UTF-8 */
    protected readonly TextDecode: (input?: ArrayBufferView | ArrayBuffer | undefined, options?: TextDecodeOptions | undefined) => string;
    /** UTF-8 -> バイト列 */
     ...
     * @return 書き込み結果
     */
    public init = async (option: Socket.Options) => {
      // モード設定
      let result = await this.setMode(option);

      // 基本設定
      if (option.sourcePort) result = result && (await this.setSourcePort(option.sourcePort));
      if (option.destIP) result = result && (await this.setDestIP(option.destIP));
      if (option.destPort) result = result && (await this.setDestPort(option.destPort));
      if (option.ipType) result = result && (await this.setIPTypeOfService(option.ipType));
      if (option.ttl) result = result && (await this.setTTL(option.ttl));
      if (option.rxBufferSize) result = result && (await this.setRXBufferSize(option.rxBufferSize));
      if (option.txBufferSize) result = result && (await this.setTXBufferSize(option.txBufferSize));

      // ソケットのオープン
      result = result && (await this.sendCommand('Open'));
      if (this.protocol === 'TCPClient') result && (await this.sendCommand('Connect'));
      if (this.protocol === 'TCPServer') result && (await this.sendCommand('Listen'));

      // 事前にrxReadDataPointerの値を記憶
      this.rxReadDataPointer = await this.getRXReadDataPointer();

      // 割り込みハンドラー設定
      if (option.onSendOKtInterrupt) this.setInterruptHandler('SendOK', option.onSendOKtInterrupt);
      if (option.onTimeoutInterrupt) this.setInterruptHandler('Timeout', option.onTimeoutInterrupt);
      if (option.onReceiveDataInterrupt) this.setInterruptHandler('ReceiveData', option.onReceiveDataInterrupt);
      if (option.onDisconnectInterrupt) this.setInterruptHandler('Disconnect', option.onDisconnectInterrupt);
      if (option.onConnectSuccessInterrupt) this.setInterruptHandler('ConnectSuccess', option.onConnectSuccessInterrupt);
      if (option.onAllInterrupt) this.setAllInterruptHandler(option.onAllInterrupt);
     ...
     * @return 書き込み結果
     */
    public readonly sendData = (data: number[] | string) => this._sendData(data, false);
     ...
     * @return 書き込み結果
     */
    public readonly sendDataFast = (data: number[] | string) => this._sendData(data, true);
     ...
     * @hidden
     */
    protected readonly _sendData = async (data: number[] | string, noWait: boolean) => {
     ...
     * @return 受信データまたは文字列
     */
    public readonly receiveData = async () => {
     ...
     */
    public readonly setInterruptHandler = (name: Socket.Interrupt, handler: ((socket: Socket) => Promise<void>) | ((socket: Socket, extra?: number[] | string | W5500.DestInfo) => Promise<void>)) => this.interruptHandlers[name] = handler;
     ...
     */
    public readonly setAllInterruptHandler = (handler: (socket: Socket, name: Socket.Interrupt, extra?: number[] | string | W5500.DestInfo) => Promise<void>) => this.allInterruptHandler = handler;

    /** 
     * 現在の設定済みプロトコルを取得
     * @return プロトコル
     */
    public readonly getProtocol = () => this.protocol;
     ...
     * @return 書き込み結果
     */
    public readonly setMode = (option: Socket.Options) => {
      this.protocol = option.protocol;
      this.stringMode = option.stringMode || false;
      return this.ethernet.numWrite(SOCKET_MODE, BSB_SOCKET_REGISTER(this.id),
        option.protocol === null ? 0 : (
     ...
     * @return 書き込み結果
     */
    public readonly sendCommand = async (command: Socket.Command) => {
      const code = Socket.CommandCodes[command];
      if (!code) throw new Error(`Unknown Command '${command}'.`);
      if (this.protocol === null) throw new Error('Must set Socket Mode before send the command.');
      if (this.protocol.indexOf('TCP') >= 0 && 0x20 < code && code < 0x30) throw new Error(`'${command}' command is only available in UDP mode.`);
      if (this.protocol === 'UDP' && 0x01 < code && code < 0x10) throw new Error(`'${command}' command is only available in TCP mode.`);
     ...
     * @return 常にtrue
     */
    public readonly checkInterrupt = async () => {
      if (!this.ethernet.getSpiStatus()) return;
      const interrupt = await this.ethernet.numRead(SOCKET_INTERRUPT, BSB_SOCKET_REGISTER(this.id));
      if (interrupt === 0) return (this.protocol !== null);
      else await this.ethernet.numWrite(SOCKET_INTERRUPT, BSB_SOCKET_REGISTER(this.id), interrupt); //リセット

      const msgList = Object.keys(Socket.InterruptFlags).filter(msg => (interrupt & Socket.InterruptFlags[msg as Socket.Interrupt]) !== 0);
      for (let m in msgList) {
        const msg = msgList[m] as Socket.Interrupt;
        const handler = this.interruptHandlers[msg];
        if (msg === 'Timeout') this.protocol = null;
        console.info(`Found Interrupt on Socket ${this.id}: ${msg}\n`);

        if (handler === undefined && this.allInterruptHandler === undefined) continue;
        ...
        if (handler !== undefined) await handler(this, extra);
        if (this.allInterruptHandler !== undefined) await this.allInterruptHandler(this, msg, extra);
        return true;
      }
     ...
     * @return ステータス
     */
    public readonly getStatus = async (): Promise<Socket.Status | 'UNKNOWN'> => {
     ...
     * @return 書き込み結果
     */
    public readonly setSourcePort = (port: number) => this.ethernet.num2Write(SOCKET_SOURCE_PORT, BSB_SOCKET_REGISTER(this.id), port);
     ...
     * @return 書き込み結果
     */
    public readonly setDestMacAddress = (mac: string) => this.ethernet.macWrite(SOCKET_DESTINATION_HARDWARE_ADDRESS, BSB_SOCKET_REGISTER(this.id), mac);
     ...
     * @return 書き込み結果
     */
    public readonly setDestIP = (ip: string) => this.ethernet.ipWrite(SOCKET_DESTINATION_IP_ADDRESS, BSB_SOCKET_REGISTER(this.id), ip);

    /**
     * 接続元のIPアドレスを取得(TCPサーバーのときのみ)
     * @return IPv4アドレス
     */
    public readonly getDestIP = () => this.ethernet.ipRead(SOCKET_DESTINATION_IP_ADDRESS, BSB_SOCKET_REGISTER(this.id));
     ...
     * @return 書き込み結果
     */
    public readonly setDestPort = (port: number) => this.ethernet.num2Write(SOCKET_DESTINATION_PORT, BSB_SOCKET_REGISTER(this.id), port);

    /**
     * 接続元のポート番号を取得(TCPサーバーのときのみ)
     * @return ポート番号
     */
    public readonly getDestPort = () => this.ethernet.num2Read(SOCKET_DESTINATION_PORT, BSB_SOCKET_REGISTER(this.id));
     ...
     * @return 書き込み結果
     */
    public readonly setMaxSegmentSize = (size: number) => this.ethernet.num2Write(SOCKET_MAX_SEGMENT_SIZE, BSB_SOCKET_REGISTER(this.id), size);
     ...
     * @return 書き込み結果
     */
    public readonly setIPTypeOfService = (type: number) => this.ethernet.numWrite(SOCKET_IP_TYPE_OF_SERVICE, BSB_SOCKET_REGISTER(this.id), type);
     ...
     * @return 書き込み結果
     */
    public readonly setTTL = (ttl: number) => this.ethernet.numWrite(SOCKET_TTL, BSB_SOCKET_REGISTER(this.id), ttl);
     ...
     * @hidden
     */
    public readonly setBufferSize = (size: Socket.BufferSize, address: number) => {
      if ([0, 1, 2, 4, 8, 16].indexOf(size) < 0) throw new Error('Given buffer size must be 0, 1, 2, 4, 8 or 16.');
     ...
     * @return 書き込み結果
     */
    public readonly setRXBufferSize = (size: Socket.BufferSize) => this.setBufferSize(size, SOCKET_RX_BUFFER_SIZE);
     ...
     * @return 書き込み結果
     */
    public readonly setTXBufferSize = (size: Socket.BufferSize) => this.setBufferSize(size, SOCKET_TX_BUFFER_SIZE);

    /**
     * 送信バッファの空きサイズを取得
     * @return 空きサイズ
     */
    public readonly getTXFreeSize = () => this.ethernet.num2Read(SOCKET_TX_FREE_SIZE, BSB_SOCKET_REGISTER(this.id));

    /**
     * 送信バッファの書き込み開始アドレスを取得
     * @return アドレス
     */
    public readonly getTXReadPointer = () => this.ethernet.num2Read(SOCKET_TX_READ_POINTER, BSB_SOCKET_REGISTER(this.id));
     ...
     * @return 書き込み結果
     */
    public readonly setTXWritePointer = (pointer: number) => this.ethernet.num2Write(SOCKET_TX_WRITE_POINTER, BSB_SOCKET_REGISTER(this.id), pointer);

    /**
     * 受信データの長さを取得
     * @return 長さ
     */
    public readonly getRXReceiveSize = () => this.ethernet.num2Read(SOCKET_RX_RECEIVE_SIZE, BSB_SOCKET_REGISTER(this.id));

    /**
     * 受信バッファの読み込み開始アドレスを取得
     * @return アドレス
     */
    public readonly getRXReadDataPointer = () => this.ethernet.num2Read(SOCLET_RX_READ_DATA_POINTER, BSB_SOCKET_REGISTER(this.id));
     ...
     * @return 書き込み結果
     */
    public readonly setRXReadDataPointer = (pointer: number) => this.ethernet.num2Write(SOCLET_RX_READ_DATA_POINTER, BSB_SOCKET_REGISTER(this.id), pointer);

    /**
     * 受信バッファの書き込み開始アドレスを取得
     * @return アドレス
     */
    public readonly getRXWritePointer = () => this.ethernet.num2Read(SOCKET_RX_WRITE_POINTER, BSB_SOCKET_REGISTER(this.id));
     ...
     * @return 書き込み結果
     */
    public readonly setFragment = (fragment: number) => this.ethernet.num2Write(SOCKET_FRAGMENT, BSB_SOCKET_REGISTER(this.id), fragment);
     ...
     * @return 書き込み結果
     */
    public readonly setKeepAliveTimer = (timer: number) => this.ethernet.numWrite(SOCKET_KEEP_ALIVE_TIMER, BSB_SOCKET_REGISTER(this.id), timer/5);
  }
  ...
  export namespace Socket {
    ...
    export interface Options {
      ...
      /** 全ての割り込みのイベントハンドラー */
      onAllInterrupt?: (socket: Socket, name: Socket.Interrupt, extra?: number[] | string | W5500.DestInfo) => Promise<void>;
    }

    /** ソケットの使用可能コマンド */
    export type Command = 'Open' | 'Listen' | 'Connect' | 'Disconnect' | 'Close' | 'Send' | 'SendMAC' | 'SendKeep' | 'Receive';
    ...
    /** ソケットのステータス */
    export type Status = 'Closed' | 'Init' | 'Listen' | 'SynSent' | 'SynReceive' | 'Established' | 'FinWait' | 'Closing' | 'TimeWait' | 'CloseWait' | 'LastACK' | 'UDP' | 'MACRAW';
